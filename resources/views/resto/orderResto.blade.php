    ...
    <div class="flex w-full">
        ...
        <main class="ml-40 flex-1 bg-white flex flex-col p-10 pr-0 pb-0">
            ...
            <section class="main-section flex flex-col pl-8bg-fixed bg-no-repeat bg-right-top" style="background-image: url('{{ asset('images/background.jpg') }}'); background-size: auto 100%;">
                ...
                <div id="odd-dates" class="flex overflow-x-auto scrollbar-hide gap-5 p-5" style="scroll-snap-type: x mandatory;">
                    @forelse($oddDates as $date)
                        <div class="scroll-snap-align start flex-shrink-0 bg-orange-100 rounded-xl shadow-md border-2 border-black hover:bg-orange-400 transform hover:scale-105 transition duration-300">
                            <button 
                                class="bg-orange-100 hover:bg-orange-300 text-black px-4 py-2 rounded-md shadow-md border-2 border-gray-400" 
                                onclick="fetchOrderDetails('{{ $date }}', 'odd')">
                                {{ $date }}
                    ...
                    @empty
                        <div class="bg-white p-2 rounded-xl shadow-md border-2 border-black flex flex-col items-center justify-center">
                            <h3 class="font-bold text-center">No Orders on Odd Dates</h3>
                        </div>
                    @endforelse
                </div>

                <!-- Baris ketiga: Last Customer -->
                <div id="last-customer" class="mt-8 mb-8">
                            ...
                            <div class="scroll-snap-align start flex-shrink-0 bg-orange-100 rounded-xl shadow-md border-2 border-black hover:bg-orange-400 transform hover:scale-105 transition duration-300">
                                ...
                                <div class="flex-1 bg-white shadow-md p-5 rounded-lg border border-gray-200">
                                    ...
                                    @foreach($lastCustomer->orders as $order)
                                        <div class="mb-4">
                                            <p class="text-gray-800">Menu: {{ $order->product_name }}</p>
